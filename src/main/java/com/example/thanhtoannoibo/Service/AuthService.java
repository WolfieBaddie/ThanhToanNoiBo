...
public class AuthService {
    ...
    public LoginResponse login(LoginRequest req) {
        ...
        if (user.getStatus() == UserStatus.DELETED) {
            recordLoginAttempt(rawIdentifier, user.getId(), false, "USER_DELETED", req);
            throw new RuntimeException("USER_DISABLED");
        }
        if (user.getStatus() == UserStatus.SUSPENDED || user.getStatus() == UserStatus.LOCKED) {
            recordLoginAttempt(rawIdentifier, user.getId(), false, "USER_BLOCKED", req);
        ...
        if (cred.getLockedUntil() != null && cred.getLockedUntil().isAfter(Instant.now())) {
            recordLoginAttempt(rawIdentifier, user.getId(), false, "LOCKED_UNTIL", req);
        ...
        if (!ok) {
            ...
            recordLoginAttempt(rawIdentifier, user.getId(), false, "BAD_CREDENTIALS", req);
        ...
        recordLoginAttempt(rawIdentifier, user.getId(), true, null, req);
