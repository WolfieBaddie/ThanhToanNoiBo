...
import { useNavigate } from 'react-router-dom'; // [MỚI] Import hook
import { ArrowRight, Utensils, CreditCard, ChevronRight } from 'lucide-react';

// Bỏ interface props cũ vì không cần truyền từ cha xuống nữa
export const QuickActionPanel: React.FC = () => {
    const navigate = useNavigate(); // [MỚI] Sử dụng hook điều hướng

    const actions = [
        { id: '/wallet', label: 'Nạp tiền', icon: <CreditCard size={18} /> },
        { id: '/menu', label: 'Đặt món', icon: <Utensils size={18} /> }, // [SỬA] Đổi id thành '/menu' cho đúng route
        ...
        <div className="bg-white dark:bg-slate-800 rounded-[32px] p-6 border border-slate-100 dark:border-slate-700 shadow-sm h-full">
            ...
            <div className="space-y-3">
                {actions.map((action) => (
                    <button
                        key={action.id}
                        onClick={() => navigate(action.id)} // [MỚI] Gọi navigate trực tiếp
