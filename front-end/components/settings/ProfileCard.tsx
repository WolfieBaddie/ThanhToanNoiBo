    ...
    <div className="bg-white dark:bg-slate-800 rounded-[32px] border border-slate-100 dark:border-slate-700 p-8 text-center shadow-sm sticky top-24 transition-colors">
      ...
      {isEditing ? (
          <div className="flex gap-2">
              <Button onClick={onCancel} variant="secondary" className="flex-1 py-2 text-sm bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-200">
                   <X size={16} /> Hủy
              </Button>
              <Button onClick={onSave} className="flex-1 py-2 text-sm">
                   <Save size={16} /> Lưu
              </Button>
          </div>
      ) : (
